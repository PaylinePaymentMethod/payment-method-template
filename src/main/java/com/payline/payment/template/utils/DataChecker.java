...
public class DataChecker {
    ...
    public static void verifyMinAge(String minAge) throws BadFieldException {
        if (!isEmpty(minAge)) {
            if (!isNumeric(minAge)) {
                throw new BadFieldException(TemplateCardConstants.MINAGE_KEY, "contract.errors.minAgeNotNumeric");
            }
            else if (Integer.parseInt(minAge) < 1 || Integer.parseInt(minAge) > 99) {
                throw new BadFieldException(TemplateCardConstants.MINAGE_KEY, "contract.errors.minAgeWrongRange");
            }
        }
    }
    ...
    public static void verifyCountryRestriction(String countryRestriction) throws BadFieldException {
        if (!isEmpty(countryRestriction )) {
            ...
            if (!isISO3166(countryRestriction)) {
                throw new BadFieldException(TemplateCardConstants.COUNTRYRESTRICTION_KEY , "contract.errors.countryNotISO");
            }
        }
    }
